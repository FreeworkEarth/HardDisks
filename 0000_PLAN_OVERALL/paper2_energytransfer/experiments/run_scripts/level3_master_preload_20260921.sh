#!/usr/bin/env bash
# ##CHRIS 2026-09-21: Level 3, v2 -- WITH THE SPRING PRE-LOADED.
# v1 (level3_master_20260920) ran clean, 600/600, and measured the wrong thing. --spring-eq was set
# to the wall start, so the spring began at its natural length while the gas was already pushing on
# it with F = N kT Z/L = 1.575 kT/sigma. The wall was released into that unbalanced load and rang:
# seed-mean swing 7.5 sigma, peak spring energy 14 kT, against the 1.04 kT the piston launched
# beyond W_qs at u = 0.2. That transient is identical in every seed, so ensemble averaging does not
# remove it -- it IS the signal v1 measured. Fix: anchor the spring at wall_start + F/k = 33.65 so
# it balances the standing gas force at t = 0. Verified on one seed: swing 7.17 -> 3.32 sigma.
#
...
BIN=./00ALLINONE
P=experiments_energy_transfer/level3_master_preload_20260921
SEEDS=${SEEDS:-40}
JOBS=${JOBS:-9}
...
run_cell(){                       # $1 = cell dir, $2 = k [kT/sigma^2], $3 = M_s, $4 = u, $5 = steps
  ...
  for i in $(seq 0 $((SEEDS - 1))); do
    ...
    ( HD_PISTON_EVENTS="$d/ev_${sd}.csv" nice -n 5 "$BIN" --mode=edmd \
        ...
        --wall-mass-factors=$ms --spring-k-sigma=$k --spring-wall=0 --spring-eq=33.65 \
