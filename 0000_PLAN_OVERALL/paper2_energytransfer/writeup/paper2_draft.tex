...
\emph{Open; v4 pending.} Geometry C: one gas of 100 disks drives a spring-loaded wall through a
...
\textbf{Status.} The ledger closes (median $4.6\times10^{-10}$, 593/600 below $10^{-8}$) and the
coherent peak stands $6.8$--$31\sigma$ above the pedestal. A parameter-free one-degree-of-freedom
model $M\ddot s = F_{\mathrm{ad}}(L - x_p + s) - F_{\mathrm{ad}}(L) - k s$ reproduces the
\emph{settled} wall displacement to $8$--$10\,\%$ but under-predicts the \emph{peak} by
$19$--$57\,\%$. The residual is confined to the transient; its origin is under test (v5). A
no-push control run in the same apparatus reproduces most of that peak excess with no gas driving
at all, so the peak statistic is a diagnostic here and not a pass criterion.
