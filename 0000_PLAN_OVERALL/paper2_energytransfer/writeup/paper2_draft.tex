...
with validated windows, $\bar s = 0.8705 \pm 0.0049\,\sigma$. A parameter-free
one-degree-of-freedom model
$M\ddot s = F_{\mathrm{ad}}(L - x_p + s) - F_{\mathrm{ad}}(L) - k s$ with the bulk equation of state
predicts it to $+3.6 \pm 0.6\,\%$. That residual is the finite-size stiffness of the 100-disk box,
the same effect Paper~1 measures as $+1\,\%$ in $c_s$, but it is not independently pinned by this
comparison and no attribution is claimed; the quantitative close follows from a direct measurement
of $F(L)$ on a held wall. The transient excess reported earlier was a peak statistic, reproduced by
a no-push control with no gas driving at all.
