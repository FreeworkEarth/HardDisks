...
\subsection{Why the 2026-09-19 pilot could not have passed}\label{sec:l3ceiling}

Quasi-statically the gas and the spring are two springs in \emph{series}. A piston compression
$\Delta x$ divides between them and the spring takes
\begin{equation}
x_{\mathrm{eq}} = \Delta x\,\frac{k_{\mathrm{gas}}}{k + k_{\mathrm{gas}}},\qquad
E_{\mathrm{qs}} = \tfrac12 k\,x_{\mathrm{eq}}^2
= \tfrac12 k \Delta x^2 \frac{k_{\mathrm{gas}}^2}{(k+k_{\mathrm{gas}})^2}.
\label{eq:series}
\end{equation}
This is \emph{maximal} at $k = k_{\mathrm{gas}}$ --- a stiffer spring barely moves, a softer one
stores nothing --- and the maximum is
\begin{equation}
E_{\mathrm{max}} = \frac{k_{\mathrm{gas}}\Delta x^2}{8}
 = \frac{N \kT f^2\,(Z + \eta Z')}{8},\qquad f = \frac{\Delta x}{L}.
\label{eq:ceiling}
\end{equation}
The capture is \textbf{second order in the compression fraction} $f$ while $W_{\mathrm{qs}}$ is first
order. That is the whole story: at $f = 0.1$, $N_s = 50$, $\eta = 0.10$, Eq.~\eqref{eq:ceiling}
gives $E_{\mathrm{max}} = 0.098\,\kT$, and the wall carries $\kT/2$ of thermal energy in that same
coordinate. The pilot ran $k = 5$, a factor $99$ stiffer than $k_{\mathrm{gas}} = 0.0507$, so its
prediction was $E_{\mathrm{qs}} = 0.0039\,\kT$ --- $128\times$ below the pedestal. \emph{No choice of
$k$ rescues it}: the ceiling over all $k$ is still $5\times$ below. The pilot's measured
$\epsilon = 0.487$ was the pedestal, not a capture. Its numbers stand as the design finding.

Two ways out, and Eq.~\eqref{eq:ceiling} says which is cheap. Raising $f$ works but only as $f^2$:
$E_{\mathrm{max}} = \kT/2$ needs $f \approx 0.16$ at $N = 100$ or $f \approx 0.07$ at $N = 500$.
The cheaper route is to stop asking the spring to catch the \emph{quasi-static} part at all.

\subsection{Where Level 3 actually lives: the acoustic pulse}\label{sec:l3acoustic}

Level 2 measured what a stepped piston launches \emph{beyond} $W_{\mathrm{qs}}$: $A u^2$ with
$A_{\mathrm{step}} = 25.9 \pm 4.3$. That excess is coherent acoustic energy --- precisely the
``ordered energy before it thermalises'' this ladder exists to chase --- and it clears the $\kT/2$
pedestal at $u \ge 0.2$ ($A u^2 = 1.04, 6.5, 25.9\,\kT$ at $u = 0.2, 0.5, 1.0$). The wall is then a
mass-loaded acoustic termination and the share a pulse can deposit is
\begin{equation}
\mathcal T_{\mathrm{imp}} = \frac{4 Z_w Z_g}{(Z_w + Z_g)^2},\qquad
Z_g = \rho\,c_s H,\qquad Z_w = M_s \omega_w = \sqrt{M_s (k + k_{\mathrm{gas}})},
\label{eq:imp}
\end{equation}
maximal at $Z_w = Z_g$.

\textbf{A trap this calculation exposed.} In geometry C of the master box ($N = 100$ over
$L = 78.5\,\sigma$) $k_{\mathrm{gas}} = 0.0246$, \emph{half} the pilot's $0.0507$ --- doubling both
$N$ and $L$ halves $k_{\mathrm{gas}} \propto N/L^2$. And with $M_s = 200$, Eq.~\eqref{eq:imp} is
already matched at $k \approx 0$: $Z_w/Z_g = \sqrt{M_s(Z+\eta Z')/N}/c_s = 0.998$. So at
$M_s = 200$ the impedance optimum and the quasi-static optimum \emph{coincide}, both at the soft
end, and a $k$ scan alone cannot tell the two predictions apart. It also means the soft end is
unrunnable for a different reason: the wall's thermal excursion $\sqrt{\kT/(k+k_{\mathrm{gas}})}$ is
$4.5\,\sigma$ at $k = k_{\mathrm{gas}}$, comparable to the $10\,\sigma$ spring compartment.

The discriminating knob is therefore $M_s$, not $k$: Eq.~\eqref{eq:series} does not contain $M_s$ at
all, so a scan at fixed $k$ leaves the quasi-static prediction flat while Eq.~\eqref{eq:imp} predicts
an interior maximum. At $k = 0.5$ the match sits at $M_s \approx 9.4$.

\begin{run}[What we run (2026-09-20)]
Master box, geometry C (divider removed, one gas of 100 disks, $\eta = 0.1001$,
$\Delta x = 7.96\,\sigma$ = 10\,\%), step protocol, 40 seeds per cell, event log on, trace
$200\,\sigma$-time past the stop.
\textbf{(a) $k$ ladder:} $k \in \{0.05, 0.5, 5\}$ $\times$ $u \in \{0.1, 0.2, 0.5, 1.0\}$.
\textbf{(b) $M_s$ ladder} at fixed $k = 0.5$: $M_s \in \{2, 10, 50, 1000\}$ $\times$
$u \in \{0.2, 0.5, 1.0\}$, which brackets the impedance match at $M_s \approx 9.4$ while
$E_{\mathrm{qs}}$ stays fixed at $0.035\,\kT$.
Primary observable is the \textbf{ensemble-mean} $E_{\mathrm{spring}}(t)$; the per-seed maximum is
the wrong estimator and the pilot showed why (it is $\sim 3\times$ the coherent peak, because it
samples the tail of the pedestal).
\end{run}
\begin{pass}[Pass criterion (2026-09-20)]
(i) The ledger \eqref{eq:ledgerE} closes with the spring term to $10^{-8}$.
(ii) The coherent peak $E_{\mathrm{coh}}$ of the seed-averaged $E_{\mathrm{spring}}(t)$ stands more
than $3\sigma$ above the pre-push pedestal in every cell with $u \ge 0.2$.
(iii) $\epsilon_{\mathrm{coh}} = E_{\mathrm{coh}}/\avg{W_{\mathrm{in}}}$ has an interior maximum in
the $M_s$ ladder at the $M_s$ nearest the impedance match, which Eq.~\eqref{eq:series} cannot
produce. Report ring-down time and the final energy split as before.
\end{pass}
