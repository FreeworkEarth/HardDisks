...
\paragraph{Verdict (2026-09-25).} With the bulk equation of state the parameter-free model predicts
the settled wall displacement to $+3.6 \pm 0.6\,\%$ (five cells pooled, measured
$\bar s = 0.8705 \pm 0.0049\,\sigma$). The residual is the finite-size stiffness of the 100-disk
box, the same effect Paper~1 measures as $+1\,\%$ in $c_s$, but it is \emph{not independently
pinned here}, so no attribution is claimed; the transient excess reported earlier was a peak
statistic reproduced by a no-push control. \textbf{Level 3 closes on the observable; the
quantitative close follows from the measured $F(L)$ set}, which obtains
$Z_{\mathrm{box}}(\eta) = FL/(Nk_BT)$ from the momentum delivered to a held wall and the gas
temperature at the same instant, with no equation of state and no area convention assumed. The peak
statistic is retained as a diagnostic only.
