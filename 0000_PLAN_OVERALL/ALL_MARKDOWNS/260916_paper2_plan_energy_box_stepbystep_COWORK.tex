...
\midrule
A & wall $|$ empty $|$ \textbf{held} wall at $L_0$ $|$ gas $|$ piston & gas & 0, 1, 2 \\
...
that the geometry is what you think it is. Drop \cmd{--headless} and \cmd{--quiet} from any command in this document
and you get the window; nothing else changes.

\begin{howto}[One demonstration run you can watch (geometry A, Level 1/2 box)]
\begin{Verbatim}[fontsize=\scriptsize,frame=single,samepage=true]
cd hspist3
./00ALLINONE --mode=edmd --experiment=energy_transfer --edmd-acc=0 \
  ...
  --velocity-right-piston-step=0.02 --max-right-piston-travel=3.93 --auto-piston-step \
  --auto-release-after-hold --wall-hold-steps=2000 --steps=40000 --fixed-dt=0.4 \
  --energy-measurement --eff-output=wall-ke --kbt1 --seed=9200
\end{Verbatim}
50 disks each side of an immovable divider; the right piston holds, then compresses at $u = 0.02$ and stops.
\textbf{Keys:} \cmd{space} pauses, \cmd{q} or \cmd{esc} quits. Slow the piston further
(\cmd{--velocity-right-piston-step=0.005}) to watch the quasi-static limit, or raise it to 3--10 to see the
single-hit regime of Section~\ref{sec:fast} --- at $u = 10$ the push is over in 0.1\,$\sigma$-time, so lower
\cmd{--wall-hold-steps} too or you will miss it.

Use \cmd{--piston-right-protocol-mode=ramp --piston-ramp-time=40} with \cmd{./00ALLINONE\_ramp} to see the gentle
launch beside the step, which is the whole of Level~2's item~1 in one picture.
...
\begin{howto}[The two-wall spring chain (geometry D) --- Paper 2's figure 1]
\begin{Verbatim}[fontsize=\scriptsize,frame=single,samepage=true]
cd hspist3
./00ALLINONE --mode=edmd --experiment=energy_transfer --edmd-acc=0 \
  --seed-drift-order=drift-first --particles=150 --particles-boxes=50,50,50 \
  --particle-radius=0.5 --l0=39.25 --height=10 \
  ...
  --max-right-piston-travel=3.93 --auto-piston-step --auto-release-after-hold \
  --wall-hold-steps=2000 --steps=40000 --fixed-dt=0.4 --kbt1 --seed=9200
\end{Verbatim}
Three compartments of 50 disks, a heavy divider ($M = 1000$) and a lighter spring-loaded wall ($M = 200$, $k = 5$).
The piston compresses on the right; energy has to cross the heavy divider before it can reach the spring, which is
Level~4's two channels and Level~5's impedance question in one picture. Verified to start and initialise the spring
measurement on 2026-09-18; it is a demonstration, not a calibrated run.
\end{howto}
...
\begin{run}
Geometry C, $L_0 = 20$, $N_s = 50$ at $\eta = 0.196$, $H = 10$, spring $k$ chosen so $T_w \approx 5\,\tau_{\mathrm{sound}}$, $M = 200$; piston step with $u/c_s = 0.02, 0.05, 0.1, 0.2, 0.5$ at fixed travel $2\,\sigma$ (so $\tau_{\mathrm{push}}/T_w$ spans $0.1$--$3$); 25 seeds each. Log $E_{\mathrm{spring}}(t)$, $KE_{\mathrm{wall}}(t)$, $KE_{\mathrm{gas}}(t)$, $W(t)$.
\end{run}
\begin{pass}
Ledger \eqref{eq:ledgerE} closes with the spring term. $E_{\mathrm{spring,max}}$ agrees with the impulsive formula at the fastest push and with the quasi-static formula at the slowest, each within $2\sigma$ over seeds. $E_{\mathrm{spring,max}}$ is a \emph{peak}: report also the ring-down time and where the energy ends (gas heat).
\end{pass}
