...
\textbf{\cmd{--temperature} takes a COMMA-SEPARATED LIST}, one value per segment, which is how the
two gases are started at different temperatures --- and with \cmd{--kbt1} the runtime temperature is
100 and $k_B$ is scaled so $k_BT = 1$, so the list is in \emph{those} units.
\cmd{--temperature=1.25,0.75} gives $k_BT = 0.0125/0.0075$: the right ratio and eighty times the
wrong scale. Use \cmd{--temperature=125,75}. Measured: $1.2500$ and $0.7500$.
...
\begin{center}\includegraphics[width=\linewidth]{../paper2_energytransfer/experiments/final/260919_demo_geomA.png}\end{center}
...
\begin{center}\includegraphics[width=\linewidth]{../paper2_energytransfer/experiments/final/260919_demo_geomD.png}\end{center}
\end{howto}
...
\paragraph{Verdict (2026-09-22).} With the box's own measured pressure --- the force a held wall
feels in equilibrium \emph{is} the pressure of the box, by the wall theorem
$P = k_BT\rho_{\mathrm{contact}}$ with the normal pressure uniform --- the parameter-free model
predicts the settled wall displacement to $-1.3 \pm 0.6\,\%$ ($2.2\sigma$); with the bulk equation
of state, $+3.6\,\%$. The box is $2.5\,\%$ over-pressured relative to Kolafa--Rottner and
$1.0\,\%$ over-stiff in $c_s$ (Paper~1); these are two different finite-size numbers and are
reported as such. The transient excess reported earlier was a peak statistic reproduced by a
no-push control, and the peak is retained as a diagnostic only. \textbf{Level 3 closes on the
observable.}
...
\subsection{How long does the divider take? The adiabatic piston}\label{sec:l4tau}

\emph{Rewritten 2026-09-27 with the papers in hand. Every equation below was read from the printed
page, not transcribed from a summary; where what is printed differs from what was quoted to us, the
printed form is used and the difference is noted.}

\paragraph{The problem is named.} Cencini \emph{et al.}~\cite{cencini2007} open by recording that
the adiabatic piston was the subject of renewed attention ``mainly stimulated by the talk of Lieb at
the Statistical Physics Conference in 1998'' [SOURCE, p.~2]. Level 4 is that problem, in an
interacting gas.

\paragraph{SOURCE --- stage 1, the damped oscillator.} With the two gases at different pressures the
piston oscillates and is damped to a \emph{mechanical} equilibrium $P_l \simeq P_r$ but
$T_l \neq T_r$. The controlling parameter is $R = mN/M$: ``For $R < R_c$ the adiabatic oscillations
of the piston are weakly damped, while for $R > R_c$ they are over-damped, $R_c$ being
$\mathcal{O}(1)$'' [SOURCE: Cencini, p.~2]. Crosignani \emph{et al.}~\cite{crosignani1996} give the
same stage as ordinary differential equations.

\emph{Free prediction for us.} With $N_s = 50$, $R = 5.00,\ 1.00,\ 0.25$ at
$M_d = 10,\ 50,\ 200$ --- over-damped, critical, ringing. And Paper~1's divider masses $50$--$2000$
give $R = 1.00$ down to $0.03$, \emph{all} weakly damped, which is exactly why the divider mode
there is a sharp resonance rather than a relaxation. The two papers agree on one parameter.

\paragraph{SOURCE --- stage 2, the Brownian drift.} Once the oscillations are damped the system sits
``in a state of marginal equilibrium approximately along the isobar $T_l/x = T_r/(L-x)$'' and ``the
Brownian motor-like mechanism is responsible for heat transfer from the warmer to the colder chamber
mediated via the piston fluctuations. \textbf{This stage occurs on a long timescale (proportional to
$M$)}'' [SOURCE: Cencini, p.~8]. Gruber \& Piasecki~\cite{gruber1999} give the prefactor. Their
Eq.~(51), \emph{as printed}, is
\begin{equation}
\avg V = \frac{\sqrt{2\pi}}{4}\sqrt{\frac{m}{M}}
\left[\sqrt{\frac{k_BT_2}{M}} - \sqrt{\frac{k_BT_1}{M}}\right],
\label{eq:gp51}
\end{equation}
with $\sqrt{m/M}$ --- a square root, not $m/M$ --- and their Eq.~(52) gives the piston kinetic
energy from a Maxwellian at temperature $\sqrt{T_1T_2}$, so
$\avg{\tfrac12 MV^2} = \tfrac12 k_B\sqrt{T_1T_2}$.

\paragraph{DERIVATION --- the relaxation time.} On the isobar a divider displacement $\dd x$ changes
the two temperatures oppositely, $\dd(T_1-T_2) = (2T/L_c)\,\dd x$ for equal compartments. Setting
$\dd x/\dd t = \avg V$ and linearising \eqref{eq:gp51} about $T_1 = T_2 = T$,
\begin{equation}
\frac{\dd(T_1-T_2)}{\dd t} = -\frac{T_1-T_2}{\tau_T},\qquad
\tau_T = \frac{4}{\sqrt{2\pi}}\,\frac{M L_c}{\sqrt{mk_BT}} .
\label{eq:tauT}
\end{equation}
\textbf{Linear in $M$, and independent of $N$ and of the density} --- the asymmetric flux and the
friction both scale with $n$ and cancel. In our units with $L_c = 38.75$ this is
$\tau_T = 61.84\,M$, i.e. $618$, $3092$, $12\,367\,\sigma$-time at $M_d = 10, 50, 200$. Verified
numerically from \eqref{eq:gp51} as printed: the coefficient comes out $61.836$ at every mass.

\paragraph{OPEN --- and larger than expected.} Gruber \& Piasecki assume an ideal gas, and Cencini
\emph{et al.} state plainly that they ``assume that the gases follow the perfect gas law''
[SOURCE, p.~2]. Ours does not: at $\eta = 0.1013$, $Z = 1.2399$. The isobar is then
$P = nZ(\eta)k_BT$ = const, giving
\begin{equation}
\dd\ln T = \left(1 + \frac{\eta Z'}{Z}\right)\dd\ln L ,
\end{equation}
a coefficient of $1.2281$ rather than $1$. That is a \textbf{$22.8\,\%$ correction, not the few per
cent anticipated}, and it \emph{shortens} $\tau_T$ to $50.35\,M$: $503$, $2517$,
$10\,070\,\sigma$-time. A second correction, to the kinetic coefficients inside \eqref{eq:gp51}
itself, is genuinely open --- the friction and the collisional asymmetry are computed there for a
Maxwellian ideal gas, and we have not redone them for hard disks. Both figures are quoted below and
the experiment should distinguish them.

\paragraph{Why an interacting gas is the interesting case, and the caveat.} Cencini \emph{et al.}
write: ``Clearly, the ultimate goal would be to quantitatively understand the behavior of the system
for an interacting gas, but this seems to be still too ambitious. Indeed only very few studies
analyzed the case of gas composed by interacting particles'' [SOURCE, p.~2]. Their own tractable
model is a \emph{randomized gas}, in which the gas is re-drawn from equilibrium after every piston
collision so that ``shock waves are ruled out''. A real interacting gas approximates that
re-equilibration physically rather than by fiat, which is the argument for measuring it here --- but
it also breaks their perfect-gas assumption, which is the $22.8\,\%$ above. Both should be said.

\paragraph{RESOLVED (2026-10-02) --- does Eq.~\eqref{eq:gp51} itself change for hard disks?}
\textbf{Consistent with unchanged.} Argued no at leading order below, and the data now say so:
the measured $\tau_T(M_d=10) = 491 \pm 29$ selects the pair \emph{(Eq.~\eqref{eq:gp51} unchanged,
hard-disk isobar $\times 1.2281$)} at $-0.6\sigma$ and excludes the ideal-gas pair at $3.5\sigma$.
An exactly compensating change --- Eq.~\eqref{eq:gp51}'s coefficient altered by $1/1.2281$ with no
isobar correction --- is not excluded by this number, but it has no thermodynamic motivation: the
isobar correction is exact thermodynamics, not a modelling choice. The argument: the drift is a ratio of the asymmetric momentum
flux to the friction, and both are proportional to the contact density $nZ$, which cancels. What
survives depends only on the disks that strike the divider having a Maxwellian velocity
distribution and uncorrelated successive collisions --- molecular chaos at the wall --- and both
hold at $\eta = 0.1$. So the hard-disk correction enters through the \emph{isobar} alone, and the
prediction to write down is
\begin{equation}
\tau_T \simeq 50.4\,M\quad\text{(hard-disk isobar)},\qquad 61.8\,M\quad\text{(ideal gas)} ,
\end{equation}
a $23\,\%$ gap between two lines. With fits good to $\pm10\,\%$ that is a discriminating
measurement, which is the point of running it.

\paragraph{DERIVATION --- the end of stage 1, and what enters stage 2.} Each gas follows its own
adiabat and the two pressures balance. For an ideal 2D gas the adiabat is $TL$ = const, so
$L_1/L_2 = \sqrt{T_1(0)/T_2(0)}$ and the divider offset is $L_c(r-1)/(r+1)$ with
$r = \sqrt{T_1/T_2}$: for $1.25/0.75$ that is $\mathbf{4.92\,\sigma}$, not the $5.3$ quoted to us
(which would need $L_c = 41.7$). Carrying KR --- adiabat $\dd\ln T = -Z\,\dd\ln L$, pressure balance
with $Z(\eta)$ --- gives
\begin{equation}
\text{offset} = \mathbf{3.99\,\sigma},\qquad -18.9\,\% \text{ against the ideal gas},
\end{equation}
and this is the value to quote. The temperatures at the end of stage 1 are $T_1 = 1.108$,
$T_2 = 0.860$, so the difference entering stage 2 is $0.249$ --- \textbf{exactly half} the initial
$0.5$, as anticipated.

\paragraph{The $\sqrt T$ nonlinearity is negligible.} At the start of stage 2 the exact
Eq.~\eqref{eq:gp51} differs from its linearisation by $0.20\,\%$ ($0.80\,\%$ at the initial
$1.25/0.75$). Integrating the exact ODE and fitting an exponential over $3\tau$ returns
$507.5$ and $2537.3\,\sigma$-time at $M_d = 10, 50$, against the analytic $507.6$ and $2537.9$ ---
agreement well under $1\,\%$. A pure exponential fit is therefore safe. (An earlier attempt that
fitted over $40\tau$ returned a spurious $1.4\times$; the tail underflows and its logarithm is
noise. Fit over a few $\tau$.)

\paragraph{DERIVATION --- the seed-to-seed spread is the physics, not noise.} Hard disks carry no
potential energy, so a compartment's kinetic temperature \emph{is} its total energy and can change
only through collisions with the divider. The spread of $T_1-T_2$ across seeds is therefore the
equilibrium fluctuation of the energy split, reached by the same Brownian exchange that relaxes the
mean. Microcanonically $f = E_1/E \sim \mathrm{Beta}(N,N)$, so
$\sigma_f = 1/(2\sqrt{2N+1}) = 0.0497$ and, with $T_1-T_2 = (2f-1)\cdot 2\bar T$,
\begin{equation}
\sigma(T_1-T_2) = 4\bar T\sigma_f = 0.199 .
\end{equation}
\textbf{Measured: $0.204$ and $0.179$.} Relaxation and fluctuation are one Ornstein--Uhlenbeck
process --- the mean decays as $e^{-t/\tau_T}$ while the variance grows as
$\sigma^2(1-e^{-2t/\tau_T})$. This is fluctuation--dissipation, and it has a practical consequence:
a \emph{step-relaxation} measurement of $\tau_T$ is intrinsically seed-hungry, while the
\emph{equilibrium autocorrelation} $\sigma^2 e^{-t/\tau_T}$ carries the same $\tau_T$ from any
record with a free divider and no step at all. Record length, not seed count, is then the binding
constraint.

\paragraph{The stage-1 overshoot is a small result, not a confirmation.} An over-damped oscillator
does not overshoot, and $R = 5$ at $M_d = 10$ is over-damped by Cencini's criterion --- yet the
divider swings to $+6.4\,\sigma$ against an equilibrium of $\approx 4$. The criterion is built on
kinetic friction into a gas that re-thermalises instantly; in a closed finite box the divider's
energy goes into sound modes and comes back, which is what Level 3's A4 control already showed.
\textbf{Stage 1 in an interacting gas is less damped than the ideal-gas criterion predicts.}

\paragraph{The stage-1 offset, quoted properly.} Taking the centre of the oscillation over the
first three periods ($T_{\mathrm{osc}} = 2\pi\sqrt{M/2k_{\mathrm{gas}}} = 44.0$ and $98.4\,\sigma$-time)
and correcting for the stage-2 decay during that window (mean of $e^{-t/\tau_T}$ over $0..T$ is
$(\tau_T/T)(1-e^{-T/\tau_T})$, i.e. factors $0.880$ and $0.944$):
\begin{center}\small
\begin{tabular}{@{}lccc@{}}
\toprule
$M_d$ & offset (corrected) & vs KR $3.989$ & vs ideal $4.922$ \\
\midrule
10 & $3.678 \pm 0.174$ & $1.8\sigma$ & $7\sigma$ \\
50 & $3.771 \pm 0.139$ & $1.6\sigma$ & $8\sigma$ \\
\bottomrule
\end{tabular}
\end{center}
The hard-disk correction to the isobar is therefore \textbf{confirmed in the stage-1 offset}: the
measurement sits with KR and $7$--$8\sigma$ from the ideal gas.

\paragraph{What the earlier $M^2$ claim was.} Withdrawn twice over. The literature says linear. And
the anchor it rested on was a different process: the A4 number was measured in the \emph{spring}
geometry, which has one gas and a spring-loaded wall, so its $\tau$ is the wall equilibrating with
its own gas and there is no $T_1-T_2$ in it at all. Comparing that with \eqref{eq:tauT} is a
category error; the factor 67 between them is its size.

\paragraph{RESULT --- $\tau_T$ from equilibrium fluctuations (2026-09-29).} Relaxation and
fluctuation are one process, so the \emph{stationary} autocorrelation carries $\tau_T$ and no
temperature step is needed --- which removes the signal-to-noise decay that defeated the step
protocol. 20 seeds at $M_d = 10$, $10\,195\,\sigma$-time each, no step and no piston. The premise
checks out first: $\sigma(T_1-T_2) = \mathbf{0.1980}$ measured against the Beta$(N,N)$ prediction
$0.199$.

The autocorrelation is \textbf{not} a single exponential --- it falls to zero by lag $45$ and
returns to $0.66$ by lag $90$. Its spectrum has $45\,\%$ of the variance at periods
$> 200\,\sigma$-time (the isobaric mode $\tau_T$ describes), a discrete peak at period
$\mathbf{95.3}$ carrying $\approx 30\,\%$, and a fast remainder. Fitting one exponential across all
three gave $288$ and $335$ from the two observables --- a $3.5\sigma$ disagreement between two
observables of the same process, which is how the error was caught. Block-averaging at
$100\,\sigma$-time removes the oscillation by construction and the two agree:
$302 \pm 65$ and $291 \pm 55$.

Calibrating the identical procedure on synthetic Ornstein--Uhlenbeck series of known $\tau$ (same
record, blocking, fit window and seed count) gives a response slope of $0.335$ --- the estimator
compresses differences threefold at this record length. Inverting it,
\begin{equation}
\tau_T(M_d = 10) = 490 \pm 190 \ \ (T_1-T_2), \qquad 450 \pm 165 \ \ (\text{divider }x),
\end{equation}
against $503$ for hard disks and $618$ ideal: $-0.1\sigma$ and $-0.6\sigma$. \textbf{The first
direct measurement of the stage-2 timescale, consistent with both hypotheses} --- a $23\,\%$ gap
cannot be resolved with a $35$--$40\,\%$ error bar. Unlike stage 1, no preference between KR and the
ideal gas is claimed here.

\paragraph{CLOSED (2026-09-30) --- the period-95 mode is Paper 1's divider resonance.} It was never
unexplained; the wrong limit of our own equation was applied to it. Rom\'an's eigenmode satisfies
$\cot K = \alpha K$ with $\alpha = M/(2N_s m)$, and here $\alpha = 10/100 = \mathbf{0.1}$ --- a
\emph{light} divider, where the gas is not a massless spring but a standing wave carrying most of
the inertia. The first branch gives $K = 1.428870$ (close to the $\alpha \to 0$ limit $K \to \pi/2$),
and with KR at $\eta = 0.100051$ ($c_s = 1.744337$) and $L_{\mathrm{eff}} = 38.75 - 2r = 37.75$,
\begin{equation}
\nu = \frac{c_s K}{2\pi L_{\mathrm{eff}}} = 0.010508 \quad\Longrightarrow\quad
T = \mathbf{95.16}\ \sigma\text{-time}.
\end{equation}
Measured, as the spectral peak with its FWHM as the error: $\mathbf{95.13 \pm 5.15}$ ($T_1-T_2$) and
$\mathbf{95.22 \pm 7.04}$ (divider $x$) --- $0.03\,\%$ and $0.06\,\%$ [DERIVATION]. The ideal-gas
variant is $117.38$, $4.3\sigma$ away, so \textbf{the mode discriminates KR from the ideal gas on
its own}; Paper~1's measured $c_s$ ($+1.01\,\%$) gives $94.21$, and the ACF fit with $\omega$ free
returns $93.27 \pm 0.51$ and $94.31 \pm 0.57$, so the period estimators agree only at the $2\,\%$
level and no claim finer than ``KR, not ideal'' is made.

\textbf{Why $\omega = \sqrt{2k_{\mathrm{gas}}/M}$ does not apply here.} That is the $\alpha \gg 1$
limit, where $\cot K \approx 1/K$ gives $K \approx \alpha^{-1/2}$. At $\alpha = 0.1$ it would give
$K = 3.162$, which is not even on the branch whose root is $1.4289$: the asymptotic form is
inapplicable, not merely inaccurate. Its prediction was $44.71$. The ``effective mass 45 rather than
10'' noted earlier is exactly the standing-wave inertia the full solution accounts for --- the
symptom, not a puzzle. \textbf{Paper~1 runs $\alpha = 0.5$--$20$ and this run $\alpha = 0.1$: one
eigenvalue problem, two papers, opposite ends of $\alpha$.}

\paragraph{RESULT --- the ACF modelled rather than blocked (2026-09-30).} Because $\omega$ is now
known, the autocorrelation has a known form and the oscillation can be \emph{modelled} instead of
averaged away --- keeping the whole record, which is what drove the block estimator's bias to
$0.335$. Fitting $C(t) = A e^{-t/\tau_T} + B e^{-t/\tau_r}\cos\omega t$ with $\omega$ fixed:
\begin{center}
\begin{tabular}{lcccc}
\toprule
observable & $A$ & $\tau_T$ & $B$ & $\tau_r$ \\
\midrule
$T_1-T_2$   & $0.472 \pm 0.024$ & $311 \pm 34$ & $0.503 \pm 0.021$ & $195 \pm 17$ \\
divider $x$ & $0.468 \pm 0.023$ & $316 \pm 38$ & $0.511 \pm 0.021$ & $186 \pm 15$ \\
\bottomrule
\end{tabular}
\end{center}
with fit rms residuals $0.034$ and $0.026$: the two components split the variance almost exactly in
half and the two observables agree on every parameter. Calibrated the same way (synthetic OU plus a
matched AR(2) oscillation, same record and seeds) the response slope is $0.438$, giving
$\tau_T = 482 \pm 100$ and $488 \pm 107$, i.e. $-0.5\sigma$ and $-0.3\sigma$ from $503$ and
$-1.9\sigma$ and $-1.6\sigma$ from $618$.

\textbf{Decision by the pre-registered rule: $0.438 < 0.6$, so the block estimator stays the
headline} and the modelled fit is reported as corroboration --- $482$ and $488$ against the block
estimator's $490 \pm 190$ and $450 \pm 165$, two analyses of one record with different biases
agreeing. The conclusion is unchanged: $\tau_T(M_d=10)$ is of order $500\,\sigma$-time and the
$23\,\%$ gap is not resolved.

\paragraph{DERIVATION --- $\tau_r$ is Paper~1's linewidth at a mass Paper~1 never ran.} For a
damped-cosine ACF the spectrum is a Lorentzian of fractional width $\Delta f/f = 1/(\pi\tau_r\nu)$,
so the measurement is $\Delta f/f = 0.155$ and $0.163$, $Q = 6.4$ and $6.1$. Paper~1 uses Mansour's
piston form $\Delta f/f = \Gamma L_y\sqrt{2/(\hat M N)}$, $\hat M = M + mN/3$; extrapolating to
$M = 10$ is the only free step, since $L_y = 10$, $N = 100$ and $\eta = 0.10$ are shared. With
Enskog $\Gamma = 0.331$ that is $\Delta f/f = 0.0711$, $\tau_r = 426$. \textbf{The measured
mode damps $2.2\times$ faster than Mansour's piston form} --- a correspondingly broader line,
$\Gamma_{\mathrm{implied}} = 0.72$, $0.76$. Reported as a
measurement, not a discrepancy: $M_d = 10$ is a factor 5 below Paper~1's lightest mass and in the
opposite $\alpha$ regime, and Paper~1's own inferred $\Gamma$ at $\eta = 0.10$ scatters from $0.084$
to $0.439$ across its nine masses. OPEN.

\paragraph{KOA design --- record length, not seeds.} The earlier ``$\approx 2300$ seeds'' was
derived for a \emph{decaying step}; the equilibrium route has no step to lose, and what hides the
$23\,\%$ gap is the estimator's bias, which is common-mode across seeds and therefore immune to
them. Separation between the two hypotheses for one experiment of the stated size:
\begin{center}
\begin{tabular}{rrrcc}
\toprule
record [$\sigma$] & $L/\tau$ & seeds & slope & separation \\
\midrule
$8\,200$ (run) & 16 & 20 & 0.29 & $0.8\sigma$ \\
$8\,200$ & 16 & 80 & 0.33 & $1.7\sigma$ \\
$16\,400$ & 33 & 20 & 0.78 & $1.7\sigma$ \\
$\mathbf{32\,800}$ & \textbf{65} & \textbf{80} & \textbf{0.85} & $\mathbf{3.3\sigma}$ \\
$65\,600$ & 130 & 20 & 0.93 & $2.5\sigma$ \\
\bottomrule
\end{tabular}
\end{center}
Quadrupling the record buys more than quadrupling the seeds. \textbf{Recommended:}
$M_d \in \{10, 50, 200\}$, records $\approx 65\,\tau_T$ ($32\,800$, $164\,000$,
$656\,000\,\sigma$-time), 80 seeds each --- at $M_d = 10$ about $13\times$ this run, an overnight
job. The mass ladder is what tests the \emph{exponent}; this run has one mass and tests neither it
nor Eqs.~\eqref{eq:gp51}--(52).

\textbf{And use the modelled estimator there, not the block one.} Its slope clears the $0.6$
threshold once the record is long enough, and at the recommended size it wins outright:
\begin{center}
\begin{tabular}{rrrcc}
\toprule
record [$\sigma$] & $L/\tau$ & seeds & slope & separation \\
\midrule
$8\,200$ (this run) & 16 & 20 & 0.45 & $1.4\sigma$ \\
$16\,400$ & 33 & 20 & 0.56 & $1.6\sigma$ \\
$32\,800$ & 65 & 20 & 0.74 & $1.9\sigma$ \\
$\mathbf{32\,800}$ & \textbf{65} & \textbf{80} & \textbf{0.74} & $\mathbf{4.0\sigma}$ \\
\bottomrule
\end{tabular}
\end{center}
against $3.3\sigma$ for the block estimator at the same cost. See
\texttt{260929\_paper2\_level4\_fd.md} and \texttt{260930\_paper2\_level4\_fd2.md}.

\paragraph{PREREQUISITE, now built and gated --- \texttt{--trace-every=N}.} The energy-transfer
trace was the only writer in \texttt{00ALLINONE.c} with no cadence control: it ignores
\texttt{--output-dt} and emits one row per step, which is 21\,GB for a single $10\,000\,\sigma$-time
cell and makes the $656\,000\,\sigma$-time record above impossible. \texttt{--trace-every=N}
decimates in \emph{steps} (exact and seed-independent); $N \le 1$ is the historical behaviour, so
every existing script is byte-identical. The guard wraps the row write only --- \texttt{recorded\_steps++}
stays outside it, because it is also the loop's termination counter and decimating it would shorten
the run. \textbf{GATE PASS}: energy\_transfer 6/6 and speed\_of\_sound 9/9 data files byte-identical
across 3 seeds, every \texttt{summary.csv} column identical, \emph{no} \texttt{EXPECT\_DIFF} needed.
Functionally, \texttt{--trace-every=300} turns 12\,000 rows (4.41\,MB) into 40 (15\,kB), every kept
row byte-identical to the full run's every-300th.

\paragraph{KOA design --- Route B, stationary heat conduction. BLOCKED, recorded, not built.} The
alternative to any decaying signal is a stationary one: hold the outer walls at $T_{\mathrm{hot}}$
and $T_{\mathrm{cold}}$, reach a steady state, read the conductivity from the steady flux. A
stationary observable has no signal-to-noise decay at all. \textbf{The current binary cannot do
it}: in \texttt{edmd.h} the heat bath is a single global --- \texttt{heatbath\_enabled} (int) and
\texttt{heatbath\_temperature} (double) --- applied to every thermalising wall, and
\texttt{S->heat\_bath} accumulates \emph{one total}. With both walls at one temperature the net
steady flux is zero by construction, and the per-side fluxes are not separable from one accumulator
even in principle. What it would need: (i) \texttt{heatbath\_temperature} as a per-wall array parsed
from a comma list, copying the \texttt{--temperature} per-segment parser at
\texttt{00ALLINONE.c:4579}; (ii) \texttt{S->heat\_bath} as a per-wall accumulator so flux in at the
hot wall and out at the cold wall are measured separately, their difference being the steady-state
check; (iii) trace columns for each. That is a core change to the energy ledger. It is \textbf{not
made}, and if it ever is, Level 0's bit-identity gate and the health contract must both be re-run
against it.

\subsection{SUPERSEDED --- how long does the divider take? (written before the papers arrived)}
\label{sec:l4tau-superseded}

\begin{center}\fbox{\begin{minipage}{0.92\linewidth}
\textbf{SUPERSEDED by \S\ref{sec:l4tau}}, which was rewritten on 2026-09-27 with Cencini \emph{et
al.} and Gruber--Piasecki in hand. The $M^2$ law derived below is \textbf{withdrawn}: the literature
states stage 2 is linear in $M$, and the $A4$ anchor this section leans on was measured in the
spring geometry --- one gas, a spring-loaded wall, no $T_1-T_2$ in it at all --- so it is a
different process. The section is kept because it is what was predicted \emph{before} the runs, and
the record of that matters. Nothing below should be quoted.
\end{minipage}}\end{center}

\paragraph{SOURCE --- not available.} The Crosignani \emph{et al.} paper was cited as attached but
is not in the repository, and Gruber--Piasecki (1999) is not either. What follows is therefore
\emph{derived here} from kinetic theory, with the one literature-dependent step marked OPEN. The
nearest paper we do hold, Gradenigo \emph{et al.}, \emph{Phys.\ Rev.\ E} \textbf{89}, 042105 (2014),
is on the same adiabatic-piston problem and should be read before this section is finalised.

\paragraph{DERIVATION --- the velocity relaxation.} A divider of mass $M$ spanning height $h$ in a
gas of number density $n$, particle mass $m$, temperature $T$, moving at velocity $V \ll
\sqrt{\kT/m}$. In the divider frame the incoming flux on one face is $n(v_x - V)$ for $v_x > V$ and
each collision transfers $2m(v_x-V)$, so
\begin{equation}
F(V) = 2mnh\left[\int_V^\infty (v_x-V)^2 f\,\dd v_x - \int_{-\infty}^V (V-v_x)^2 f\,\dd v_x\right].
\end{equation}
This vanishes at $V=0$; differentiating and using $\int_0^\infty v f\,\dd v = \sqrt{\kT/2\pi m}$,
\begin{equation}
\gamma \equiv -\frac{\dd F}{\dd V}\bigg|_0 = 8mnh\sqrt{\frac{\kT}{2\pi m}}
 = 4nh\sqrt{\frac{2m\kT}{\pi}},\qquad \tau_v = \frac{M}{\gamma}.
\label{eq:gamma}
\end{equation}
So the velocity relaxation is \textbf{linear in $M$}, as quoted. For the two-compartment Level 4 box
($N_s = 50$, $L = 38.75$, $h = 10$), $nh = 1.290$ and $\gamma = 4.118$, giving
$\tau_v = 2.4,\ 12.1,\ 48.6\,\sigma$-time at $M = 10,\ 50,\ 200$.

\paragraph{DERIVATION --- the temperature relaxation does \emph{not} inherit it.} Our own
measurements settle this without any fitting. In the A4 spring geometry ($nh = 1.274$,
$\gamma = 4.066$) the validated fit gives $\tau = 46.1 \pm 1.2$ at $M = 50$ and $\gtrsim 680$ at
$M = 200$, against $\tau_v = 12.3$ and $49.2$. The \emph{ratio} $\tau/\tau_v$ is then $3.7$ and
$13.8$ --- it is itself proportional to $M$. Hence
\begin{equation}
\tau_T \;\sim\; \frac{M}{c\,m}\,\tau_v \;=\; \frac{M^2}{c\,m\,\gamma},\qquad c \approx 14 ,
\label{eq:tauT-superseded}
\end{equation}
with $c$ the same to within $7\,\%$ at both masses. The physical content is the standard two-stage
adiabatic-piston picture: pressures equalise mechanically on $\tau_v$, but each collision exchanges
only $O(m/M)$ of the divider's energy, so temperatures equalise a further factor $\sim M/m$ later.

\paragraph{Check that costs nothing.} $\tau(200)/\tau(50) \ge 14.8$ measured, against $4.0$ for
$\tau \propto M$ and $16.0$ for $\tau \propto M^2$. Extrapolating the $M = 50$ anchor by $M^2$ gives
$46.1 \times 16 = 738\,\sigma$-time, against the measured lower bound $\gtrsim 680$. The $M^2$ law
is used to set the record lengths below; it is \emph{not} fitted to them.

\paragraph{OPEN.} Three things this section does not settle. (i) The constant $c \approx 14$ is
empirical --- the two-stage argument fixes the \emph{exponent}, not the prefactor, and whether $c$
should carry $N_s$ or $\sqrt{N_s}$ is exactly what the literature would say. (ii) At $M = 10$ the
$M^2$ law predicts $\tau_T = 1.8 < \tau_v = 2.4$, which is impossible: the asymptotic law must
saturate near $M \sim c\,m$, and $M = 10$ is deliberately placed there to see it. (iii) The spring
geometry and the two-compartment geometry differ in $nh$ by $1.3\,\%$; the predictions transfer with
$\gamma$, which assumes the mechanism is the same in both, and Level 4 tests that.

\paragraph{Predictions for the runs.} $\tau_T \approx 2.4$ (saturated) at $M_d = 10$, $45.5$ at
$50$, $729$ at $200$. Records $\ge 5\tau_T$: 1000 $\sigma$-time at $M_d = 10$ and $50$ (40 seeds),
4000 at $M_d = 200$ (8 seeds, to measure $\tau(200)$ rather than bound it).

\paragraph{Level 4 v3 result (2026-09-26).} 88 runs, 0 aborts, 0 health. The work splits in half
and the divider takes half the compression at $M_d = 10, 50, 200$ --- $0.1$--$1.5\sigma$ and
$0.1$--$1.3\sigma$ respectively, with no mass dependence over a factor 20. The error bars are set by
the per-gas kinetic-energy fluctuation, $\sqrt{N_s} = 7.1\,\kT$ per seed. The $\tau_T$ prediction of
\S\ref{sec:l4tau} is \textbf{untested rather than refuted}: at $u = 0.05$ the push lasts
$78.6\,\sigma$-time against $\tau_v = 2.4$--$48.6$, so the divider follows quasi-statically and both
gases end at the same length and the same temperature --- $T_1 - T_2$ wanders within
$\pm 0.09\,\kT$ and never relaxes, because it was never created. Testing $\tau_T$ needs either a
push that outruns the divider ($u \gtrsim 1.6$, $0.32$, $0.08$ at $M_d = 10, 50, 200$) or, cleaner,
the two gases started at different temperatures with no piston motion at all --- the exact analogue
of the A4 no-push control. See \texttt{260926\_paper2\_level4\_v3.md}.
...
\emph{Physica A}, vol.~268, pp.~412--423, 1999. (arXiv:cond-mat/9812359)
