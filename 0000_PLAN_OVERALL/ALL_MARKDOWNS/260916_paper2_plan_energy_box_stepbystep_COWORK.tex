...
\paragraph{OPEN --- the period-95 mode.} The adiabatic gas spring predicts a divider oscillation at
$\omega = \sqrt{2k_{\mathrm{gas}}/M_d}$ with $k_{\mathrm{gas}} = N\kT c_s^2/L^2 = 0.0988$ per side,
i.e. period $44.7$. The measured peak is at $95.3$, a factor $2.13$ too slow; an isothermal spring
gives $63.4$ and does not close it either. It is not a release transient --- the autocorrelation on
$t > 2000$ matches that on $t < 2000$, and the seed mean shows no coherent ring-down. No mechanism
is claimed; hydrodynamic added mass on a piston in a closed tube would have to supply $\approx 35m$.
It carries $\approx 30\,\%$ of the variance of both observables, so any future estimator must handle
it.
...
nor Eqs.~\eqref{eq:gp51}--(52). See \texttt{260929\_paper2\_level4\_fd.md}.
