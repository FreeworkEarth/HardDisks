...
\paragraph{Verdict (2026-09-24, v6).} The parameter-free model with bulk equation-of-state inputs
predicts the settled wall displacement to $+3.6 \pm 0.6\,\%$ (five cells pooled, $6.2\sigma$). With
the box's own measured standing force, its Paper-1 sound speed and Level~1's measured adiabat as
inputs --- all measurements, none fitted --- the residual is $-0.1 \pm 0.7\,\%$, $0.1\sigma$.
\textbf{Level 3 passes on $\epsilon_{\mathrm{settled}}$.} The transient excess reported in v4 was a
peak statistic, reproduced by a no-push control run, and the peak is retained as a diagnostic only.
Both papers then say one thing: the $N = 100$ box is about one per cent stiffer than bulk in $c_s$,
and once that is known its energy transfer is predicted.
