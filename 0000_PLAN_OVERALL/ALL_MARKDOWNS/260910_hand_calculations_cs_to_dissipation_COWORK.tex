...
A disk of diameter $\sigma$ sweeps a ``collision strip'' of width $\sigma$ (two centres collide when their distance is $\sigma$). In the dilute limit the rate at which one particle collides is number density $\times$ strip width $\times$ mean relative speed; at finite density multiply by the contact value of the pair correlation, $g(\sigma)$:
\begin{equation}
\nu_c = n\,\sigma\, g(\sigma)\,\avg{v_{\mathrm{rel}}}
= n\sigma g(\sigma)\sqrt{\frac{\pi\kT}{m}} .
\end{equation}
...
\begin{equation}
\boxed{\;\nu_c = \frac{2\,(Z-1)}{\sqrt\pi}\;\sqrt{\frac{\kT}{m}}\,\frac1\sigma\;}
\qquad\xrightarrow{\;\eta\to0\;}\qquad \nu_c \simeq \frac{4\eta}{\sqrt\pi}.
\label{eq:nuc}
\end{equation}
This is the ``physical collision rate'' we used to interpret the event budget: at $\eta = 0.69$, $Z \approx 10.1$ gives $\nu_c \approx 10.3$ collisions per particle per $\sigma$-time.

\subsection{Mean free path}
\begin{equation}
\lambda = \frac{\avg v}{\nu_c} = \frac{1}{\sqrt2\, n\sigma\, g(\sigma)}
\qquad\xrightarrow{\;\eta\to0\;}\qquad
\lambda = \frac{\pi}{4\sqrt2}\,\frac{\sigma}{\eta} = 0.555\,\frac{\sigma}{\eta}.
\label{eq:lambda}
\end{equation}
(In the chat I quoted $0.278\,\sigma/\eta$; that missed a factor 2. The conclusions below do not change --- every Knudsen number just doubles --- but use \eqref{eq:lambda}.)

\subsection{Knudsen number along route A}
The Knudsen number compares the mean free path with the container:
\begin{equation}
\mathrm{Kn} = \frac{\lambda}{L_0} = \frac{0.555\,\sigma/\eta}{3.927\,\sigma/\eta} = 0.141
\qquad\text{independent of }\eta\ \text{(dilute-limit }g\text{; at fixed }N=100).
\label{eq:Kn}
\end{equation}
The $\eta$ cancels because both $\lambda$ and $L_0$ scale as $1/\eta$ when $N$ is fixed. Along route A every point has the same ratio ``collisions per crossing'', about $1/\mathrm{Kn} \approx 7$ collisions while a particle crosses the compartment.

\begin{worked}[{Worked numbers at the dilute end}]
At $\eta = 0.0196$ ($L_0 = 200$): $\lambda = 28\,\sigma$, $\nu_c = 0.046$ per $\sigma$-time (using $Z = 1.040$). The hold before release is $2000\times0.4/24 = 33.3$ $\sigma$-time, \ie $33.3\times0.046 \approx 1.5$ collisions per particle. That is enough only because the velocities are drawn Maxwellian and rescaled to $\kT = 1$ at $t = 0$; the hold does not \emph{create} equilibrium, it preserves it.
\end{worked}
...
$\lambda/\sigma$ & 28 & 57 & 141 & 283\\
$\mathrm{Kn} = \lambda/L_0$ & 0.14 & 0.28 & 0.71 & 1.4\\
\bottomrule
\end{tabular}

Between $N_{\mathrm{side}} = 25$ and $10$ the gas stops being a fluid (particles cross the compartment without colliding). This is the regime change of Section~\ref{sec:collisionless}, and it is why ``fewer particles'' is a different experiment from ``lower density''.
\end{worked}
