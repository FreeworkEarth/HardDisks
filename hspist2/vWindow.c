...
#include <OpenGL/glext.h> // Add this line to include the necessary header for glTextureRangeAPPLE
#include <sys/time.h>
#include "glgraph.h"
#include "rstuff.h"
#include "globals.h"
...
Uint32 videoTimer(Uint32 interval, void* param);
...
{
    Wnd.texEnum = GL_TEXTURE_RECTANGLE;
...
void videoNewWindow()				// create texture, video-in destination, start videoTimer()
{
    ...
    SDL_AddTimer(0, videoTimer, NULL);	// Do the first poll straight away
...
void videoUpdateFrame(void* pData, long dataSize)
{
    ...
    if (!firstex) {		// set parameters, upload the first image
        // glTextureRangeAPPLE(Wnd.texEnum, dataSize, pData);  // Removed
        // glTexParameteri(Wnd.texEnum, GL_TEXTURE_STORAGE_HINT_APPLE , GL_STORAGE_SHARED_APPLE);  // Removed
...
Uint32 videoTimer(Uint32 interval, void* param)
{
    ...
    videoDisplay();				// call our main display program!
    updatekeys();
    return interval;
}