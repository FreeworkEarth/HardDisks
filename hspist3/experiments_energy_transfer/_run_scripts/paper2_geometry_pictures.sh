...
#   0 .. 89.50 (--l0=44.75), wall_S centre 10.5, divider centre 50.25, thickness 1.0 (default).
#   empty spring compartment 0 .. 10.0 | gas 1 11.0 .. 49.75 | gas 2 50.75 .. 89.50, 38.75 each.
set -uo pipefail
...
COMMON=(--show-simulation --demo --mode=edmd --edmd-acc=0 --seed-drift-order=drift-first
        --particle-radius=0.5 --l0=44.75 --height=10
...
C=("${GAS1[@]}" --wall-mass-factors=200 --spring-k=5 --spring-wall=0 --spring-eq=10.5
   --eff-output=spring --max-right-piston-travel=7.96)
D=("${GAS2[@]}" --wall-mass-factors=200,1000 --spring-k=5 --spring-wall=0 --spring-eq=10.5
