...
P=experiments_energy_transfer/level4b_transmission_20261008b
MAXJOBS=${MAXJOBS:-9}
...
# FAIL CLOSED. The previous filter was [c for c in want if c in e.columns], which silently dropped
# any misspelled column -- it dropped W0_v (asked for as "W0_v_sigma") and with it the divider
# kinetic energy and the exact per-seed ledger. A missing column now stops the reduction by name.
missing = [c for c in want if c not in e.columns]
if missing:
    sys.exit("REDUCTION ABORTED: columns absent from %s: %s" % (sys.argv[1], ",".join(missing)))
keep = want
e[keep].to_csv(sys.argv[2], index=False)
PY
