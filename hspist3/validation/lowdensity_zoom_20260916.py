...
P = T.PLOTS
OUT = os.path.join(P, "260916_cs_vs_eta_lowdensity_zoom")
XMAX = 0.15
...
by = defaultdict(list)
for r in csv.DictReader(open(os.path.join(P, "260916_A2_cs_per_mass.csv"))):
...
for N in (900, 1600):
    ...
    ax.errorbar([p[0] for p in pts], [p[1] for p in pts], yerr=[p[2] for p in pts], fmt="o", color=col,
                ...
                label=f"{lab} · 5 masses × 10–35 seeds, 37.5 periods")
...
ax.grid(True, ls=":", alpha=0.6); ax.legend(loc="upper left", fontsize=8.5, framealpha=0.95)
