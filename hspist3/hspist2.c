...
/*his code appears to simulate a system where hard disks interact within a confined environment, influenced by the motion of pistons. The simulation includes various physical phenomena like the velocity and kinetic energy of particles, and the collisions between particles and the walls of the container. Here’s an overview of what the key elements in the code do:

Key Variables:
	•	Position & Velocity Arrays: Arrays like X, Y, VX, and VY represent the positions and velocities of the spheres (or particles) in the simulation. These arrays are updated in each iteration of the simulation.
	•	Histograms: The program collects various statistics (like velocity distributions and collision statistics) using histogram arrays (Histo, Histo1, Histo2, Histowall).
	•	Piston Control: The xpist1, xpist2, vpist1, and vpist2 variables represent the positions and velocities of the pistons, which move and create pressure within the system.
	•	Kinetic Energy: Ktot, KE0, KE1, KE2 store kinetic energy values at different stages of the simulation.
	•	Bumps & Collisions: The bump structure tracks collisions and interactions between particles and the walls or other particles. The bumps array contains information about these interactions.
	•	Simulation Control Flags: Variables like slowctl, keyslowctl, ncount, and nruns manage simulation pacing, iteration count, and control over certain display behaviors.

Functions:
	•	cleanup(): This function, currently just a placeholder, should contain any cleanup logic, such as freeing memory or closing files.
	•	Display(): This function controls the main simulation display. It updates particle positions, handles piston movements, updates histograms, and refreshes the screen with the updated display. It also includes conditional logic for various simulation modes (e.g., adjusting piston velocities).

Notes:
	•	The code uses the SDL2 library for graphical rendering, which allows the program to visualize the simulation in a window. It may also support full-screen display depending on the FULLSCR macro.
	•	The simulation likely involves some form of statistical mechanics and gas dynamics, where the particles (disks) are modeled as hard spheres, and their interactions are governed by physical laws like collisions and pressure.

This code appears to be part of a larger program designed to explore the statistical properties of hard disk gases or similar systems. You can use it as a base for further analysis or simulation, especially if you need to modify parameters like piston speed or particle density to explore different configurations.*/
...
void Display(SDL_Window* _window, SDL_Renderer* _renderer)
{
    ...
    for (i = 0; i < slowctl; i++)
    {
		...
		if (X == NULL || Y == NULL || VX == NULL || VY == NULL) {
			printf("Error: One of the arrays (X, Y, VX, VY) is NULL\n");
			return;
		}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < 10; i++) {
        double g1, g2;
        gaussrand(&g1, &g2);
        printf("Random values %d: g1=%f, g2=%f\n", i, g1, g2);
    }
    ...
    printf("Before initgauss: nrad1=%ld, nrad2=%ld, rad1=%f, rad2=%f\n", nrad1, nrad2, rad1, rad2);
	fflush(stdout);
	initgauss(nrad1, nrad2, RAD1, 0);
	printf("After initgauss\n");
	fflush(stdout);
	printf("Before rowl calc: XW1=%f, XW2=%f, YW1=%f, YW2=%f, LS=%f\n", XW1, XW2, YW1, YW2, LS);
	fflush(stdout);	

    rowl = ceil((XW2 - XW1) / LS);
    nrows = (YW2 - YW1) / LS;
    ncells = rowl * nrows;
	printf("Before zeroing histo, NENTS=%d\n", NENTS);
	fflush(stdout);	
	...
	int counter = 0;
	while (counter < 1000)  // Allow 1000 frames before exiting
	{
    ...
    //{
		...
		if (!_window || !_renderer)
			{
				printf("ERROR: SDL Window or Renderer is NULL!\n");
				return 1;
			}
		

		if (X == NULL || Y == NULL || VX == NULL || VY == NULL) {
			printf("Error: Arrays X, Y, VX, or VY are uninitialized.\n");
			return 1;
		}

        Display(_window, _renderer);

		printf("Returned from Display\n");
    	fflush(stdout);
		counter++;
